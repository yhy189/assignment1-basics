...
def build_report(
    ...
    return {
        ...
        "pairwise": {
            ...
            "mla_vs_moe": _pairwise(mla, moe),
            "mla_vs_muon": _pairwise(mla, muon),
        },
        "ranking": {
...
def _to_text(report: dict[str, Any]) -> str:
    ...
    for name in ["dense", "moe", "muon", "mla"]:
        run = runs[name]
        lines.extend(
            [
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare dense/moe/muon/mla metrics JSON files")
